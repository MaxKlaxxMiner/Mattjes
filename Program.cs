...
namespace Mattjes
{
  class Program
  {
    ...
    static void Main(string[] args)
    {
      ...
      Console.WriteLine("    --- weiße Schachs ---");
      Console.WriteLine();
      var marker = new List<int>();
      for (int p = 0; p < 64; p++) if (b.IsChecked(p, Piece.White)) marker.Add(p);
      PrintMarkedBoard(b, marker);
      marker.Clear();

      Console.WriteLine();
      Console.WriteLine("    --- schwarze Schachs ---");
      Console.WriteLine();
      for (int p = 0; p < 64; p++) if (b.IsChecked(p, Piece.Black)) marker.Add(p);
      PrintMarkedBoard(b, marker);
